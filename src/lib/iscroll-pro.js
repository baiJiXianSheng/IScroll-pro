var _prototype = HTMLElement.prototype;

_prototype.addClass = function (classnames) {
    classnames = classnames ? classnames.trim() : "";

    var str = this.getAttribute("class") || "";

    if (str.indexOf(classnames) > -1) return this;

    this.setAttribute("class", (str + " " + classnames).trim());

    return this;
}

_prototype.removeClass = function (classnames) {
    classnames = classnames ? classnames.trim() : "";

    var arr2 = classnames.split(" "),
        str = this.getAttribute("class") || "",
        arr1 = str.split(" ");

    for (var i = 0, len = arr2.length; i < len; i++) {
        for (var j = 0, jLen = arr1.length; j < jLen; j++) {
            if (arr1[j] === arr2[i]) {
                arr1.splice(j, 1);
                j--;
                if (j < 0)
    ...
    this.setAttribute("class", arr1.join(" "));

    return this;
}

_prototype.hasClass = function (classname) {
...
_prototype.data = function (data, value) {
    if (typeof data !== "string")
        return console.error("data arg must be a string !");

    data = data.trim();
    var index = data.indexOf("data-");
    if (!value)
        return index === 0 ? this.getAttribute(data) : this.getAttribute("data-" + data);

    if (index > -1)
        this.setAttribute(data, value);
    else
        return console.error("data arg must start with 'data-' !");
...
/**
 * 设置元素属性
 * @param {String|Object} attr 属性名 
 * @param {String} value 属性值
 */
_prototype.attr = function (attr, value) {
    // arguments.callee.caller 指向该函数调用者
    // arguments.callee(); // 指向该函数本身
    if (attr instanceof Object && !value) {
 ...
 * @param {String} value 
 */
_prototype.css = function (name, value) {
    if (value != undefined) 
...
_prototype.html = function (value) {
    return value != undefined ? (this.innerHTML = value, this) : this.innerHTML;
}
...
// dom.style.top 只能读取行内样式
_prototype.width = function () {
    var obj = getComputedStyle(this),
        width = _getNumber(obj.width);

    width += (obj.paddingLeft == "auto" ? 0 : _getNumber(obj.paddingLeft)) + (obj.paddingRight == "auto" ? 0 : _getNumber(obj.paddingRight)) + (obj.marginLeft == "auto" ? 0 : _getNumber(obj.marginLeft)) + (obj.marginRight == "auto" ? 0 : _getNumber(obj.marginRight));
...
_prototype.height = function () {
    var obj = getComputedStyle(this),
        height = _getNumber(obj.height);
        
    height += (obj.paddingTop == "auto" ? 0 : _getNumber(obj.paddingTop)) + (obj.paddingBottom == "auto" ? 0 : _getNumber(obj.paddingBottom)) + (obj.marginTop == "auto" ? 0 : _getNumber(obj.marginTop)) + (obj.marginBottom == "auto" ? 0 : _getNumber(obj.marginBottom));
...
;(function (window, document, Math) {
    ...
    // INSERT POINT: OPTIONS
            ...
            loadingFn: function (fn) { fn() } // 加载（刷新/加载更多）时触发的函数
        };
        ...
        if (this.options.showRefreshStatus) {
            var div1 = document.createElement("div"),
                div2 = document.createElement("div"),
                div3 = document.createElement("div");
            div1.attr("id", "refresh-loading-status").addClass("init-status");
            div1.appendChild(div2);
            div1.appendChild(div3);

            this.refreshDom = div1;
            this.wrapper.appendChild(div1);
        }
        if (this.options.showLoadMoreStatus) {
            var div1 = document.createElement("div"),
                div2 = document.createElement("div"),
                div3 = document.createElement("div");
            div1.attr("id", "loadmore-loading-status").addClass("init-status");
            div1.appendChild(div2);
            div1.appendChild(div3);

            this.loadMoreDom = div1;            
            this.wrapper.appendChild(div1);
        }

        this.refreshDomHeight = this.refreshDom ? _getNumber(this.refreshDom.height()) : 42;
        this.loadMoreDomHeight = this.loadMoreDom ? _getNumber(this.loadMoreDom.height()) : 42;
    ...
    // INSERT POINT: _refresh
        ...
        setLoadingPosition: function (type) {
            // 下拉刷新
            if (type == 1) 
            {
                var top = this.refreshDomHeight;
            ...
            {
                var bottom = this.loadMoreDomHeight;
